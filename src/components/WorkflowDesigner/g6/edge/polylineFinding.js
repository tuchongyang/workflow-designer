...
const getConnectablePoints = function (sBBox, tBBox, sPoint, tPoint) {
  ...
  let points = [...vertexOfBBox(sLineBBox), ...vertexOfBBox(tLineBBox), ...vertexOfBBox(outerBBox)]
...
const crossPointsByLineAndBBox = function (bbox, centerPoint) {
  ...
  if (!(centerPoint.x < bbox.minX || centerPoint.x > bbox.maxX)) {
    crossPoints = [...crossPoints, { x: centerPoint.x, y: bbox.minY }, { x: centerPoint.x, y: bbox.maxY }]
  }
  if (!(centerPoint.y < bbox.minY || centerPoint.y > bbox.maxY)) {
    crossPoints = [...crossPoints, { x: bbox.minX, y: centerPoint.y }, { x: bbox.maxX, y: centerPoint.y }]
  }
